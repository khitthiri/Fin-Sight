...
async function callFlask(endpoint, userId) {
  ...
  const res = await axios.post(`${AI_URL}${endpoint}`, payload, {
    timeout: 15000,
...
router.post('/insights', protect, async (req, res) => {
  try {
    const data = await callFlask('/predict/insights', req.user._id)
    res.json(data)
  } catch (err) {
    console.error('AI insights error:', err.message)
    res.status(503).json({ message: 'AI service unavailable: ' + err.message })
  }
})

router.post('/predict', protect, async (req, res) => {
  try {
    const data = await callFlask('/predict/expenses', req.user._id)
    res.json(data)
  } catch (err) {
...
router.post('/personality', protect, async (req, res) => {
  try {
    const data = await callFlask('/predict/personality', req.user._id)
    res.json(data)
  } catch (err) {
